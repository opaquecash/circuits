...
for url in "${URLS[@]}"; do
  ...
  if fetch "$url"; then
    ...
    echo "Note: 2^16 (65,536 constraints) comfortably fits V2 (5,421)."
    echo "If a future circuit exceeds it, re-run with a larger power,"
    echo "e.g.: bash scripts/download_ptau.sh 17"
    exit 0
  fi
  rm -f "$OUT"
done
